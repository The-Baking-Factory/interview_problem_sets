...
TOKEN = os.getenv('DISCORD_INTERVIEW_REMINDERS')
...
class Manager(commands.Cog):
    ...
    async def on_ready(self):
        ...
        if not self.prune_user_history.is_running():
            self.prune_user_history.start()
        if not self.contentmod.is_running():
            self.contentmod.start()
        if not self.reminders.is_running():
            self.reminders.start()
        if not self.roleassigns.is_running():
            self.roleassigns.start()
        if not self.censortokens.is_running():
            self.censortokens.start()
