...
API_ENDPOINT = os.getenv('API_ENDPOINT_COINS_BY_ID')
...
class Manager(commands.Cog):
    ...
    async def activity(self):
        ...
        high_24_change = round(((float(ratio_usd.replace(',',''))-float(high_24h.replace(',','')))/float(high_24h.replace(',','')))*100)
        low_24_change = round(((float(ratio_usd.replace(',',''))-float(low_24h.replace(',','')))/float(low_24h.replace(',','')))*100)
        ...
        await asyncio.sleep(3)
        await self.bot.change_presence(activity=discord.Game(name=f"24h High: ${high_24h} [{high_24_change}%]"))
        await asyncio.sleep(3)
        await self.bot.change_presence(activity=discord.Game(name=f"24h Low: ${low_24h} [{low_24_change}%]"))
        await asyncio.sleep(3)
        await self.bot.change_presence(activity=discord.Game(name=f"% from ATH: {ath_change_percentage}%"))
